...
static void undo_check_modified_step()
{
	...
	// In FLTK, we need to update the window's changed state
	// This mirrors the GTK behavior of checking and toggling modified state
	MainWindow* window = (MainWindow*)text_buffer->modify_cb_arg();
	if (window) {
...
static bool undo_undo_real()
{
	...
	if (!undo_list.empty()) {
		...
		undo_list.pop_back();
DV(printf("undo_undo_real: undo left = %d, redo left = %d\n",
	(int)undo_list.size(), (int)redo_list.size()));
...
static bool undo_redo_real()
{
	if (!redo_list.empty()) {
		...
		redo_list.pop_back();
DV(printf("undo_redo_real: undo left = %d, redo left = %d\n",
	(int)undo_list.size(), (int)redo_list.size()));
